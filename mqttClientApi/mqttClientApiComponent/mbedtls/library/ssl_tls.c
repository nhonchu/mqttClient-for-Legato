...
int mbedtls_ssl_parse_certificate( mbedtls_ssl_context *ssl )
{
    ...
    if( ( ret = mbedtls_ssl_read_record( ssl ) ) != 0 )
    {
        ...
        {
            fprintf(stdout, "ssl_read 1, error EOF");
            fflush(stdout);
            return( ret );
        }
...
int mbedtls_ssl_parse_change_cipher_spec( mbedtls_ssl_context *ssl )
{
    ...
    if( ( ret = mbedtls_ssl_read_record( ssl ) ) != 0 )
    {
        if( ret == MBEDTLS_ERR_SSL_CONN_EOF )
        {
            fprintf(stdout, "ssl_read 2, error EOF");
            fflush(stdout);
            return( ret );
        }
...
int mbedtls_ssl_parse_finished( mbedtls_ssl_context *ssl )
{
    ...
    if( ( ret = mbedtls_ssl_read_record( ssl ) ) != 0 )
    {
        if( ret == MBEDTLS_ERR_SSL_CONN_EOF )
        {
            fprintf(stdout, "ssl_read 3, error EOF");
            fflush(stdout);
            return( ret );
        }
    ...
    if( ssl->in_offt == NULL )
    {
        ...
        if( ! record_read )
        {
            if( ( ret = mbedtls_ssl_read_record( ssl ) ) != 0 )
            {
                if( ret == MBEDTLS_ERR_SSL_CONN_EOF )
                {
                    fprintf(stdout, "ssl_read error 4, EOF");
                    fflush(stdout);
                    return( ret );
                }
            ...
            ssl->in_msgtype == MBEDTLS_SSL_MSG_APPLICATION_DATA )
        {
            ...
            if( ( ret = mbedtls_ssl_read_record( ssl ) ) != 0 )
            {
                if( ret == MBEDTLS_ERR_SSL_CONN_EOF )
                {
                    fprintf(stdout, "ssl_read error 5, EOF");
                    fflush(stdout);
                    return( ret );
                }
